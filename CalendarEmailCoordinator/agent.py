...
root_agent = LlmAgent(
    ...
    2. When user asks to SEND A SIMPLE TEXT EMAIL ("Send email to shashwat saying hi"):
       - Call EmailAgent.
       - Instruction: "Draft and send a text-only email to [Person] about [Subject]."

    3. When user asks to EMAIL A FILE FROM DRIVE ("Attach resume and send", "Find report in drive and email it"):
       - Call EmailAgent.
       - Instruction: "Search for [File Name] in Drive, download it, and attach it to an email for [Person]."

    4. When user asks to JUST BOOK A CALENDAR EVENT ("Book meeting", "Check schedule"):
       - Call CalendarAgent.
       - Instruction: "Create event for [Subject] at [Time]."

    5. When user asks to BOOK MEETING AND SEND INVITE ("Schedule call with shashwat and invite him"):
       - Call MeetingAgent.
    ...
    6. When user asks for UNSUPPORTED TASKS ("What is the weather?", "Write a poem", "News updates"):
       - Do not call any tools.
